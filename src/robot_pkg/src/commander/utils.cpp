...
map<DetectionType, vector<robot_pkg::Detection>> g_vision_data;
...
void init(ros::NodeHandle nh) {
    g_detector_type_pub = nh.advertise<robot_pkg::Detector>(DESIRED_DETECTOR_TOPIC, 1000);
...
void handleNewVisionData(const robot_pkg::Detections::ConstPtr& detections) {
    ...
    for (robot_pkg::Detection det : detections->detections) {
        ...
        if (g_vision_data.find(det.detection_type) == g_vision_data.end()) {
            ...
            g_vision_data[det.detection_type] = vector<robot_pkg::Detection>();
            pthread_mutex_unlock(&g_vision_mutex);
        }
        // Add the detection to the map's vector for the type
        pthread_mutex_lock(&g_vision_mutex);
        g_vision_data[det.detection_type].push_back(det);
        pthread_mutex_unlock(&g_vision_mutex);
    }
}
...
void setDetector(DetectorType detector_type, int mode) {
    Timeout::exitCheck();
    ROS_INFO("Setting detector");

    if (detector_type == robot_pkg::Detector::NO_DETECTOR) {
        ROS_INFO("Stopping vision");
    } else {
        ROS_INFO("Changing or starting a detector");
    }

    robot_pkg::Detector detector;
...
vector<robot_pkg::Detection> getCurrentVisionData(DetectionType object_type) {
    ...
    vector<robot_pkg::Detection> ret_val;
    if (detection == g_vision_data.end()) {
        ret_val = vector<robot_pkg::Detection>();
    } else {
...
bool waitVisionData(DetectionType object_type, double timeout, vector<robot_pkg::Detection>* result,
                      unsigned int min_num_detections, unsigned int max_num_detections) {
    Timeout::exitCheck();

    long long int cur_time = getCurrentMsCount();
    vector<robot_pkg::Detection> detection = getCurrentVisionData(object_type);
...
bool waitVisionData(vector<DetectionType> object_types, double timeout, vector<robot_pkg::Detection>* result,
                      unsigned int min_num_detections, unsigned int max_num_detections) {
    Timeout::exitCheck();

    long long int cur_time = getCurrentMsCount();

    vector<robot_pkg::Detection> detections;
    for (const auto& obj_type : object_types) {
        vector<robot_pkg::Detection> detection = getCurrentVisionData(obj_type);
